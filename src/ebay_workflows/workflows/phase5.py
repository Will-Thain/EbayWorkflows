...
def run_phase5_ocr_verification(
    ...
    try:
        ...
        def _persist_analysis(listing_image: ListingImage, analysis: ImageAnalysisResult) -> None:
            nonlocal candidates_updated, embedding_updates, tier7_proposals_raw, tier7_post_veto
            if analysis.skipped:
                return
            if analysis.cascade is not None:
                tier7_proposals_raw += len(analysis.cascade.all_proposals)
                tier7_post_veto += sum(
                    1
        ...
        def _scryfall_cards_for_listing(listing_id: Any) -> list:
            candidate_rows = candidate_repo.for_listing(listing_id)
            cards = []
            seen: set[str] = set()
        ...
        def _run_parallel_real_ocr(images: list[ListingImage]) -> list[ImageAnalysisResult]:
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                futures = {}
                for img in eligible:
                    listing = listing_repo.get(img.listing_id)
                    futures[
                        executor.submit(
        ...
        all_candidates = candidate_repo.all_for_scope(sample_ids)
        candidate_repo.ensure_scryfall_cards_loaded(all_candidates)
        ...
        listings_by_id = listing_repo.by_ids(listing_ids)
